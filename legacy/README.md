...
Status: FROZEN. Cette section contient l'ancien pipeline Tkinter/CLI.
Aucune nouvelle fonctionnalité ni maintenance ici sans validation SPRINT HQ.

Pour exécuter les commandes legacy, ajoute `legacy/` au `PYTHONPATH` :

```bash
PYTHONPATH=legacy python -m motherload_projet.cli --demo
```
